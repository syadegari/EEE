...
nmlist_htessel = nml.read(f'{working_dir}/input')
nmlist_mpr = nml.read(f'{working_dir}/mpr_global_parameter.nml')
...
mpr_global_nmlist = nml.read(f'{working_dir}/mpr_global_parameter.nml')
htessel_nmlist = nml.read(f'{working_dir}/input')
...
for i, morris_set in enumerate(morris_params):
    ...
    for params, nmlist in zip([param_mpr, param_htessel],
        ...
        nmlist.nml.write(f'{nmlist.tag}_{i:{digit_format}}')
